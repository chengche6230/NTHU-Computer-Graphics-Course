...
bool magLinear = true;
bool minLinear = true;
GLuint iLocOffset, iLocIsEye, iLocDiffuseTex;

void setUniformVariables()
{
	vertex_perpixel = glGetUniformLocation(program, "vertex_perpixel");

	iLocP = glGetUniformLocation(program, "project_matrix");
	iLocV = glGetUniformLocation(program, "view_matrix");
	iLocM = glGetUniformLocation(program, "model_matrix");
	...
	iLocLightInfo[0].ambient = glGetUniformLocation(program, "light[0].Ambient");
	iLocLightInfo[0].diffuse = glGetUniformLocation(program, "light[0].Diffuse");
	iLocLightInfo[0].specular = glGetUniformLocation(program, "light[0].Specular");
	...
	iLocLightInfo[1].ambient = glGetUniformLocation(program, "light[1].Ambient");
	iLocLightInfo[1].diffuse = glGetUniformLocation(program, "light[1].Diffuse");
	iLocLightInfo[1].specular = glGetUniformLocation(program, "light[1].Specular");
	...
	iLocLightInfo[2].ambient = glGetUniformLocation(program, "light[2].Ambient");
	iLocLightInfo[2].diffuse = glGetUniformLocation(program, "light[2].Diffuse");
	iLocLightInfo[2].specular = glGetUniformLocation(program, "light[2].Specular");
	...
	// V [TODO] Get uniform location of texture
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
	{
		...
		// V [TODO] Bind texture and modify texture filtering & wrapping mode
		...
		// Eye offset
		float ox = (models[cur_idx].cur_eye_offset_idx / 4) * 0.5;
		float oy = 1 - (models[cur_idx].cur_eye_offset_idx % 4) * 0.25;
		glUniform2f(iLocOffset, ox, oy);

		glUniform1i(vertex_perpixel, per_vertex_or_per_pixel);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_G:
			magLinear = !magLinear;
			cout << "Magnification linear: " << magLinear << endl;
			break;
		case GLFW_KEY_B:
			minLinear = !minLinear;
			cout << "Minification linear: " << minLinear << endl;
			break;
		case GLFW_KEY_RIGHT:
			models[cur_idx].cur_eye_offset_idx = (models[cur_idx].cur_eye_offset_idx + 1) % models[cur_idx].max_eye_offset;
			cout << "Current eye idx: " << models[cur_idx].cur_eye_offset_idx << endl;
			break;
		case GLFW_KEY_LEFT:
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case GeoRotation:
		...
		break;
	case LightEdit:
		tmp = 0.1 * (float)yoffset;
		if (lightType == 2)
			lightInfo[lightType].spotCutoff += tmp;
		else
			lightInfo[lightType].diffuse += Vector3(tmp, tmp, tmp);
		break;
	case ShininessEdit:
		shininess += 0.1 * 50 * (float)yoffset;
		break;
	}
}
	...
	if (mouse_pressed) {
		if (starting_press_x < 0 || starting_press_y < 0) {
			...
			case GeoRotation:
				...
				break;
			case LightEdit:
				lightInfo[lightType].position.x += diff_x * 0.01;
				lightInfo[lightType].position.y -= diff_y * 0.01;
				break;
			}
		}
	}
...
GLuint LoadTextureImage(string image_path)
{
	...
	if (data != NULL)
	{
		...
		// V [TODO] Bind the image to texture
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "110065512 HW3", NULL, NULL);
    if (window == NULL)
    {
